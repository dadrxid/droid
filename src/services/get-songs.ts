  ...
  async getSongs(query: string, _playlistLimit: number, shouldSplitChapters: boolean): Promise<[SongMetadata[], string]> {
    ...
    try {
      ...
      if (YOUTUBE_HOSTS.includes(url.host)) {
        ...
        throw new Error('Spotify is not supported — use a YouTube URL or search query');
      } else {
      ...
      if (err instanceof Error && err.message.includes('Spotify is not supported')) {
        throw err;
      }

      if (err instanceof Error && err.message === 'that doesn\'t exist') {
        throw err;
      }
